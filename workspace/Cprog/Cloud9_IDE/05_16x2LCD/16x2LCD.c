...
/* Write a 8bit data to the sensor at the address addr */
...
/* Read len bytes from addr of the sensor in to the adresss pBuffer */
...
void MPU6050_Init()
{
    /* The Reset value makes the Unit to Enter Sleep Mode so disable sleep */
...
void MPU6050_ReadAclMtr(short int *pBuffer)
{
...
void MPU6050_ReadGyro(short *pBuffer)
{
    char gyro_buffer[6];
...
int main(void)
{
    ...
    while(1)
    {
        MPU6050_ReadAclMtr(acc_value);
        MPU6050_ReadGyro(gyro_value);
